...
\author{
    Ariel Edgardo Levy\\
    \texttt{alevy@bixen.ai}\\
    Bixen --- \url{https://bixen.ai}\\
...
\begin{enumerate}
    \item \textbf{Perception-Action Loop}: $\mathcal{A}$ maintains a continuous cycle with perception function $p: \mathcal{E} \rightarrow \mathcal{S}$ and action function $\alpha: \mathcal{S} \rightarrow \mathcal{E}$ where $\mathcal{E}$ is the environment and $\mathcal{S}$ is the internal state space.
    ...
    \item \textbf{Adaptive Autonomy}: $\mathcal{A}$ can modify its policy $\pi: \mathcal{S} \rightarrow \mathbb{A}$ based on accumulated experience without explicit reprogramming, where $\mathbb{A}$ is the action space.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
    ...
    \item \textbf{\map Architecture}: Hebbian learning for inter-agent routing with convergence guarantees (Section~\ref{sec:map})
...
\textbf{Part II: Architecture} (Sections 3--4 and Capabilities 1--10) details the complete \lumen implementation, from routing patterns to the \map learning system.
...
Meta-learning systems include a meta-agent that observes agent performance and adjusts routing strategies~\cite{wu2023autogen,hong2023metagpt}. This is the basis for our \map architecture.
...
The core innovation of \sgrag lies in its \textbf{speculative gap detection}, which identifies when the knowledge base lacks sufficient information to answer a query.

\begin{definition}[Entity Coverage]
\label{def:entity-coverage}
The \textbf{coverage function} measures the fraction of query entities present in retrieved documents:
\begin{equation}
\text{coverage}(q, D) = \frac{|\text{entities}(q) \cap \text{entities}(D)|}{|\text{entities}(q)|}
\end{equation}
where $\text{entities}(\cdot)$ extracts named entities (measures, dimensions, business terms) via NER.
\end{definition}
...
where $q$ is the query, $D$ is the document corpus, $\tau_{\text{conf}} = 0.75$ is the confidence threshold, and $\tau_{\text{cov}} = 0.5$ is the entity coverage threshold.
...
\section{Capability 10: MAP --- Multiagent Plasticity}
\label{sec:map}

\subsection{Motivation}

Static routing rules cannot adapt to changing usage patterns. \map addresses this through Hebbian learning principles~\cite{hebb1949organization} applied to agent coordination.

\begin{figure}[H]
    \centering
    \includesvg[width=0.85\textwidth]{10_map_evolucion_en}
    \caption{Evolution from static to plastic systems in \map}
    \label{fig:map-evolution}
\end{figure}

\begin{figure}[H]
    \centering
    \includesvg[width=0.85\textwidth]{10_map_trust_graph_en}
    \caption{Trust graph structure in \map}
...
\begin{theorem}[Weight Variance Bound with Plasticity]
\label{thm:variance-bound}
Under \map with learning rate $\eta > 0$ and decay $\lambda \in (0,1)$, the equilibrium variance of trust weights is bounded:
\begin{equation}
\text{Var}[w_{ij}] \leq \frac{\eta^2 \sigma^2}{\lambda(2-\lambda)}
\end{equation}
where $\sigma^2$ is the variance of success signals, assumed i.i.d.\ with bounded second moment.
\end{theorem}
...
\begin{corollary}[Policy Variance Bound]
\label{cor:policy-variance}
The routing policy $\pi_t = \text{softmax}(\mathbf{w}_t)$ satisfies:
\begin{equation}
\text{Var}[\pi_t] \leq \frac{\eta^2 \sigma^2}{8 \cdot \lambda(2-\lambda)}
\end{equation}
since softmax gradients satisfy $\sum_j (\partial \pi_i / \partial w_j)^2 \leq 2\pi_i^2(1-\pi_i)^2 \leq 0.125$ (maximum at $\pi_i = 0.5$), and by the delta method $\text{Var}[\pi] \leq 0.125 \cdot \text{Var}[w]$.
\end{corollary}

\begin{corollary}[Practical Bound]
For typical hyperparameters $\eta = 0.01$, $\lambda = 0.1$, and $\sigma^2 \approx 0.25$:
\[
\text{Var}[w] \leq \frac{(0.01)^2 \times 0.25}{0.1 \times 1.9} = \frac{0.000025}{0.19} \approx 0.00013
\]
This yields $\text{Var}[\pi] \leq 0.125 \times 0.00013 \approx 1.6 \times 10^{-5}$, ensuring routing fluctuations below $0.4\%$ (one standard deviation), providing highly stable agent selection.
\end{corollary}

\subsection{MAP-Bench: Evaluation Protocol}

We propose MAP-Bench with 9 metrics across 3 dimensions:
...
\bottomrule
\end{tabular}
\caption{MAP-Bench metrics and targets}
\label{tab:map-bench}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{\map Architecture}: Hebbian learning for adaptive agent routing with provable convergence bounds
...
\begin{enumerate}
    \item Extension of \map to cross-organization learning
    \item Integration of formal verification for safety-critical applications
    \item Development of domain-specific MAP-Bench variants
\end{enumerate}
...
An agent $\mathcal{A}$ is a tuple $(\mathcal{S}, \mathbb{A}, \mathcal{T}, \pi, \rho)$ where:
\begin{itemize}
    \item $\mathcal{S}$: State space
    \item $\mathbb{A}$: Action space
    \item $\mathcal{T}$: Tool set
    \item $\pi: \mathcal{S} \rightarrow \mathbb{A}$: Policy function
    \item $\rho: \mathcal{S} \times \mathbb{A} \rightarrow \mathcal{S}$: Transition function
\end{itemize}
\end{definition}
...
For the weight vector $\mathbf{w} \in \mathbb{R}^{|I| \times |A|}$ with independent success signals, each component $w_{ij}$ evolves independently under the same dynamics. Thus the bound holds component-wise:
\[
\text{Var}[w_{ij}] \leq \frac{\eta^2 \sigma^2}{\lambda(2-\lambda)}
\]
...
\begin{proof}[Proof of Corollary~\ref{cor:policy-variance}]
The routing policy $\pi_t = \text{softmax}(\mathbf{w}_t)$ maps weights to probabilities. The softmax Jacobian has entries:
\[
\frac{\partial \pi_i}{\partial w_j} = \pi_i(\delta_{ij} - \pi_j)
\]
where $\delta_{ij}$ is the Kronecker delta.

\textbf{Bounding the gradient norm.} We compute:
\[
\sum_j \left(\frac{\partial \pi_i}{\partial w_j}\right)^2 = \pi_i^2(1-\pi_i)^2 + \pi_i^2 \sum_{k \neq i} \pi_k^2
\]
Since $\sum_{k \neq i} \pi_k = 1 - \pi_i$ and by convexity $\sum_{k \neq i} \pi_k^2 \leq (1-\pi_i)^2$, we have:
\[
\sum_j \left(\frac{\partial \pi_i}{\partial w_j}\right)^2 \leq 2\pi_i^2(1-\pi_i)^2
\]
The function $f(x) = 2x^2(1-x)^2$ achieves its maximum at $x = 0.5$: $f(0.5) = 2 \times 0.25 \times 0.25 = 0.125$.

\textbf{Delta method.} By the delta method for variance propagation:
\[
\text{Var}[\pi_i] \approx \sum_j \left(\frac{\partial \pi_i}{\partial w_j}\right)^2 \text{Var}[w_j] \leq 0.125 \cdot \text{Var}[w]
\]
Substituting the weight variance bound from Theorem~\ref{thm:variance-bound}:
\[
\text{Var}[\pi_t] \leq \frac{0.125 \cdot \eta^2 \sigma^2}{\lambda(2-\lambda)} = \frac{\eta^2 \sigma^2}{8 \cdot \lambda(2-\lambda)}
\]
\end{proof}
...
\begin{itemize}
    ...
    \item \textbf{Sweet spot}: $\eta = 0.01$, $\lambda = 0.1$, $\sigma^2 = 0.25$ yields $\text{Var}[w] \approx 0.00013$ and $\text{Var}[\pi] \approx 1.6 \times 10^{-5}$
\end{itemize}
\end{remark}
...
\begin{itemize}
    \item Python 3.13.1
    \item FastAPI 0.115.6
    \item Ray 2.53.0
    \item Azure OpenAI API version 2025-01-01-preview
    \item Weaviate 1.28.2
    \item Redis 7.4.2
\end{itemize}
