...
Under \pema with learning rate $\eta < \frac{1}{2\lambda_{\max}}$ and decay $\lambda > 0$, the variance of routing decisions is bounded:
\begin{equation}
\text{Var}[\pi_t] \leq \frac{\eta^2 \sigma^2}{2\lambda - \eta^2 \lambda_{\max}^2}
\end{equation}
where $\sigma^2$ is the variance of success signals and $\lambda_{\max}$ is the maximum eigenvalue of the intent covariance matrix.
\end{theorem}
...
For typical hyperparameters $\eta = 0.01$, $\lambda = 0.1$, $\sigma^2 \approx 0.25$, and $\lambda_{\max} \approx 1$, the bound yields $\text{Var}[\pi] \leq 0.013$, ensuring routing fluctuations below 12\%.
\end{corollary}
...
Consider the weight update dynamics:
\[
...
At stationarity, $\mathbb{E}[w_\infty] = \frac{\eta \mu}{\lambda}$.
...
At equilibrium:
\[
\text{Var}[w_\infty] = \frac{\eta^2 \sigma^2}{1-(1-\lambda)^2} = \frac{\eta^2 \sigma^2}{2\lambda - \lambda^2}
\]

For multi-dimensional case with covariance structure, the bound becomes:
\[
\text{Var}[\pi_t] \leq \frac{\eta^2 \sigma^2}{2\lambda - \eta^2 \lambda_{\max}^2}
\]

provided $\eta < \frac{1}{2\lambda_{\max}}$ for stability.
\end{proof}
...
Core architectural patterns and reference implementation available at:
\url{https://github.com/innerbi/lumen}
